...
const data: FullServicePageData = {
  ...
  h1: "畳工事・畳張替えなら埼玉畳店｜表替え・裏返し・新調・原状回復まで対応",
  ...
  serviceOverview:
    ...
    "業者選びでは、価格の安さだけで判断しないことが大切です。使用する畳表の産地・等級、縁の種類、廃材処分の有無、施工後の保証など、見積もりの条件を揃えて比較しましょう。埼玉畳店では、現地確認のうえで畳床の状態まで見極め、本当に必要な工事だけを正直にご提案します。住宅から旅館・寺社・店舗・賃貸管理物件まで、規模を問わず対応いたします。",
  workScope: [
  ...
  templeProposal:
    "寺社では、本堂・客殿・控室それぞれの格式に合わせた畳工事をご提案します。本堂には高品質な畳表と伝統的な紋縁を選び、法要・年忌・祭礼の日程から逆算して確実に間に合わせる計画施工を行います。境内での作業では養生と清掃を徹底し、神聖な空間にふさわしい静かで丁寧な施工を心がけます。客殿や控室は来客の印象を左右するため、清潔感のある国産い草で整えるのが効果的です。広い本堂の大量枚数も一括で対応し、格式を損なわない仕上がりを目指します。",
  storeProposal:
